...
- **Exagrow Case Study & Testimonial Spot**: Updated Exagrow card on `portfolio.html` and dataset in `app.js` with live deployment URL (`https://exagrow.com`), WordPress-to-modern-stack migration narrative, clean technology tags (`Netlify / Cloudflare`, `Custom Architecture`, `High-Converting UI`), and a pre-styled commented-out testimonial spot (`<!-- CLIENT TESTIMONIAL PLACEHOLDER -->`) with soft keyline borders avoiding double amber border collisions.
