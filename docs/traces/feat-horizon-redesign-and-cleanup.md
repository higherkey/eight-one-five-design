...
- **Top Hero Sweep Alignment**: Extended `.hor-hero::before` to `top: -5rem` and `height: calc(100% + 5rem)` so the sand diagonal background sweep connects seamlessly under the fixed dark navigation bar, eliminating top white space gaps.
